...
class GeneralK8sClient {
    ...
    async createDeployment(deploymentManifest) {
        let result;
        const response = await this.client.apis.apps.v1.namespaces('default').deployments.post({ body: deploymentManifest });
        if (response.statusCode && (response.statusCode >= 200 && response.statusCode <= 299)) {
    ...
    async deleteDeployment(deploymentName) {
        let result;
        const response = await this.client.apis.apps.v1.namespaces('default')
    ...
    async createConfigMap(configMapManifest) {
        let result;
        const response = await this.client.api.v1.namespaces('default')
